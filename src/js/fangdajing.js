...
window.onload = function () {
    ...
    $(window).scroll(function(){
            if($(this).scrollTop()>=contentRtop){
                $('.contentRtop').css({position:'fixed',top:0,background:'white'})
            }else{
                $('.contentRtop').css({position:'static'})
            }

            if($(this).scrollTop()>=uls){
                $('.contentRmainR>ul').css({position:'fixed',top:162,background:'#f5f5f5',width:'98'})
               
            }
            if($(this).scrollTop()<=uls ){
                $('.contentRmainR>ul').css({position:'static'})
            }
            if($(this).scrollTop()>=$('.contentRmainR').height()){
                $('.contentRmainR>ul').css({position:'relative',top:592})
            }
        ...
        addcartbtn[i].onclick = function(){
            ...
            for (let k = 0; k < products.length; k++) {
                if(product.name==products[k].name){
                    ...
                    products[k].num = products[k].num+1;
                    // products[k].price = productsPrice+productPrice
                    setStorage(products);
                    return;
                }   
            }
